...
angular.module('restApp').controller('ObjectTypeController', function($scope, $window, objectTypeService, $q, $http) {
    $scope.newEntity = {
        code: "",
        flag: 0
    };
    ...
    var getAllEntities = function() {        
        var promise = objectTypeService.getEntities();
    ...
    var postEntity = function() {
        var promise = objectTypeService.insertEntity($scope.newEntity);
        promise.then(function(response) {
            if (response.status === 200) {
    ...
    var putEntity = function() {
        var promise = objectTypeService.updateEntity($scope.entitySelect);
            promise.then(function(response) {
            if (response.status === 200) {
                $scope.newEntity.code = "";
                $scope.newEntity.flag = 0;
