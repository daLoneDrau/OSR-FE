...
angular.module('restApp').controller('DiceController', function($scope, $window, dieService, diceService, $q, $http) {
    $scope.newEntity = {
        code: "",
        die: {},
        number: 0
    ...
    var getAllDieEntities = function() {        
        var promise = dieService.getEntities();
    ...
    var getAllEntities = function() {        
        var promise = diceService.getEntities();
        promise.then(function(response) {
            console.log("GET::");
            console.log(response);
            if (response.status === 200) {
    ...
    var postEntity = function() {
        ...
        var promise = diceService.insertEntity($scope.newEntity);
        promise.then(function(response) {
            if (response.status === 200) {
    ...
    var putEntity = function() {
        var promise = diceService.updateEntity($scope.entitySelect);
            promise.then(function(response) {
            if (response.status === 200) {
                $scope.newEntity.code = "";
