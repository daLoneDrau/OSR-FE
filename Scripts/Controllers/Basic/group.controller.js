...
angular.module('restApp').controller('GroupController', function($scope, groupService, $q, $http) {
    $scope.divName = "";
    ...
    var getAllEntities = function() {        
        var promise = groupService.getEntities();
    ...
    var postEntity = function() {
        var promise = groupService.insertEntity({ name: $scope.divName });
            promise.then(function(response) {
            if (response.status === 200) {
                $scope.divName = "";
                getAllEntities();
    ...
    var putEntity = function() {
        var promise = groupService.updateEntity($scope.groupSelect);
            promise.then(function(response) {
            if (response.status === 200) {
                $scope.divName = "";
                getAllEntities();
