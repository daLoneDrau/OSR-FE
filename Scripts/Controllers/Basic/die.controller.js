...
angular.module('restApp').controller('DieController', function($scope, $window, dieService, $q, $http) {
    $scope.newEntity = {
        code: "",
        value: 0
    };
    ...
    var postEntity = function() {
        var promise = dieService.insertEntity($scope.newEntity);
        promise.then(function(response) {
            if (response.status === 200) {
    ...
    var putEntity = function() {
        var promise = dieService.updateEntity($scope.entitySelect);
            promise.then(function(response) {
            if (response.status === 200) {
                $scope.newEntity.code = "";
                $scope.newEntity.value = 0;
