...
angular.module('restApp').controller('ElementController', function($scope, $window, elementService, $q, $http) {
    $scope.newEntity = {
        code: "",
        value: 0
    };
    ...
    var postEntity = function() {
        var promise = elementService.insertEntity($scope.newEntity);
        promise.then(function(response) {
            if (response.status === 200) {
    ...
    var putEntity = function() {
        var promise = elementService.updateEntity($scope.entitySelect);
            promise.then(function(response) {
            if (response.status === 200) {
                $scope.newEntity.code = "";
                $scope.newEntity.value = 0;
