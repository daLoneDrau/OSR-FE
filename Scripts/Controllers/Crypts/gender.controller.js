...
angular.module('restApp').controller('CryptGenderController', function($scope, $window, cryptGenderService, $q, $http) {
    $scope.newEntity = {
        name: "",
        description: ""
    };
    ...
    var postEntity = function() {
        ...
        var promise = cryptGenderService.insertEntity($scope.newEntity);
        promise.then(function(response) {
            if (response.status === 200) {
    ...
    var putEntity = function() {
        var promise = cryptGenderService.updateEntity($scope.entitySelect);
            promise.then(function(response) {
            if (response.status === 200) {
                $scope.newEntity.name = "";
                $scope.newEntity.description = "";
