...
angular.module('restApp').controller('GenderController', function($scope, $window, genderService, $q, $http) {
    $scope.newEntity = {
        name: "",
        description: ""
    };
    ...
    var getAllEntities = function() {        
        var promise = genderService.getEntities();
    ...
    var postEntity = function() {
        ...
        var promise = genderService.insertEntity($scope.newEntity);
        promise.then(function(response) {
            if (response.status === 200) {
    ...
    var putEntity = function() {
        var promise = genderService.updateEntity($scope.entitySelect);
            promise.then(function(response) {
            if (response.status === 200) {
                $scope.newEntity.name = "";
                $scope.newEntity.description = "";
