...
angular.module('restApp').controller('CryptDieController', function($scope, $window, cryptDieService, $q, $http) {
    $scope.newEntity = {
        code: "",
        value: 0
    };
    ...
    var postEntity = function() {
        var promise = cryptDieService.insertEntity($scope.newEntity);
        promise.then(function(response) {
            if (response.status === 200) {
    ...
    var putEntity = function() {
        var promise = cryptDieService.updateEntity($scope.entitySelect);
            promise.then(function(response) {
            if (response.status === 200) {
                $scope.newEntity.code = "";
                $scope.newEntity.value = 0;
