...
angular.module('restApp').controller('CryptGroupController', function($scope, cryptGroupService, $q, $http) {
    $scope.divName = "";
    ...
    var postEntity = function() {
        var promise = cryptGroupService.insertEntity({ name: $scope.divName });
            promise.then(function(response) {
            if (response.status === 200) {
                $scope.divName = "";
                getAllEntities();
    ...
    var putEntity = function() {
        var promise = cryptGroupService.updateEntity($scope.groupSelect);
            promise.then(function(response) {
            if (response.status === 200) {
                $scope.divName = "";
                getAllEntities();
