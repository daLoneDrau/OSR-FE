...
angular.module('restApp').controller('CryptModifierController', function($scope, $window, cryptModifierService, $q, $http) {
    $scope.newEntity = {
        code: "",
        percent: false,
        value: 0
    ...
    var getAllEntities = function() {        
        var promise = cryptModifierService.getEntities();
    ...
    var postEntity = function() {
        var promise = cryptModifierService.insertEntity($scope.newEntity);
        promise.then(function(response) {
            if (response.status === 200) {
    ...
    var putEntity = function() {
        var promise = cryptModifierService.updateEntity($scope.entitySelect);
            promise.then(function(response) {
            if (response.status === 200) {
                $scope.newEntity.code = "";
                $scope.newEntity.flag = 0;
