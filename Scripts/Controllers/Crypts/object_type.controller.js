...
angular.module('restApp').controller('CryptObjectTypeController', function($scope, $window, cryptObjectTypeService, $q, $http) {
    $scope.newEntity = {
        code: "",
        flag: 0
    };
    ...
    var postEntity = function() {
        var promise = cryptObjectTypeService.insertEntity($scope.newEntity);
        promise.then(function(response) {
            if (response.status === 200) {
    ...
    var putEntity = function() {
        var promise = cryptObjectTypeService.updateEntity($scope.entitySelect);
            promise.then(function(response) {
            if (response.status === 200) {
                $scope.newEntity.code = "";
                $scope.newEntity.flag = 0;
