...
angular.module('restApp').controller('CryptAttributeController', function($scope, $window, cryptAttributeService, cryptElementService, $q, $http) {
    $scope.newEntity = {
        name: "",
        description: "",
        code: ""
    ...
    var postEntity = function() {
        var promise = cryptAttributeService.insertEntity($scope.newEntity);
        promise.then(function(response) {
            if (response.status === 200) {
    ...
    var postElementEntity = function(name) {
        var s = [];
        s.push("ELEMENT_");
        s.push(name.toUpperCase().replace(/ /g, "_"));
        s = s.join("");
        ...
        promise.then(function(response) {
            if (response.status === 200) {
                ...
                if (!found) {
                    ...
                    console.log(elements);
                    var elPromise = cryptElementService.insertEntity({
                        code: s,
                        value: lastValue
                    });
    ...
    var putEntity = function() {
        console.log("POST::");
        console.log($scope.entitySelect);
        var promise = cryptAttributeService.updateEntity($scope.entitySelect);
            promise.then(function(response) {
            if (response.status === 200) {
                $scope.newEntity.name = "";
                $scope.newEntity.description = "";
